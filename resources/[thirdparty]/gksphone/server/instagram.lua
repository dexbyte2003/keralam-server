...
QBCore.Functions.CreateCallback("gksphone:getInstoAacc",function(source, b)
  local srcIdentifier = QBCore.Functions.GetPlayer(source)
  local identifi = srcIdentifier.identifier

  if not identifi then return; end
   exports.oxmysql:execute("SELECT * FROM gksphone_insto_accounts",{},function(e)
    local storieb = {}
		for i=1, #e, 1 do
...
AddEventHandler('gksphone:instofollow', function(username, password, id)
  local src = source
  local srcIdentifier = QBCore.Functions.GetPlayer(src)
  local identifi = srcIdentifier.identifier
  ...
  getaUserIns(username, password, function (user)
    ...
    local g={}
     exports.oxmysql:execute("SELECT * FROM gksphone_insto_accounts WHERE username = @username",{["@username"]=username},function(h)
      ...
      if h[1] ~= nil then
        ...
        g =  json.decode(h[1].takip)
          ...
          exports.oxmysql:execute("UPDATE gksphone_insto_accounts SET takip = @takip WHERE username = @username",{["@username"]=username,["@takip"]=json.encode(g)})
    ...
    local g={}
     exports.oxmysql:execute("SELECT * FROM gksphone_insto_accounts WHERE username = @username",{["@username"]=username},function(h)
      ...
      if h[1] ~= nil then
        ...
        exports.oxmysql:execute("UPDATE gksphone_insto_accounts SET takip = @takip WHERE username = @username",{["@username"]=username,["@takip"]=json.encode(test)})
...
AddEventHandler('gksphone:insto_newinstas', function (inap)
  ...
  local isImg = ext == '.png' or ext == '.jpg' or ext == '.gif' or string.sub(inap.message, -5) == '.jpeg'
    ...
    data['embeds'][1]['title'] = inap.forename .." hat einen neuen Beitrag gepostet!"
