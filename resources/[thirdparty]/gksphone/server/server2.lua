...
function getTinder(username)
	local result = exports.oxmysql:executeSync("SELECT id FROM gksphone_tinderacc WHERE username = @username", {['@username'] = username})
...
QBCore.Functions.CreateCallback("gksphone:tinderfetch",function(a,b)
	local d = QBCore.Functions.GetPlayer(a)
    if not d then 
        return 
    end;
     exports.oxmysql:execute("SELECT * FROM gksphone_tinderacc",{},function(e)
...
QBCore.Functions.CreateCallback("gksphone:tinderMatchex",function(a,b)
    local d = QBCore.Functions.GetPlayer(a)
    if not d then 
        return 
    end;
     exports.oxmysql:execute("SELECT A.user_id AS userA, B.user_id AS userB FROM gksphone_tindermatch A JOIN gksphone_tindermatch B ON A.user_id = B.friend_id AND A.friend_id = B.user_id AND A.id <> B.id WHERE A.is_match = 1 AND B.is_match = 1",{},function(e)
...
QBCore.Functions.CreateCallback("gksphone:tinderMess",function(a,b)
    local d = QBCore.Functions.GetPlayer(a)
    if not d then 
        return 
    end;
     exports.oxmysql:execute("SELECT * FROM gksphone_tindermessage",{},function(e)
...
AddEventHandler("gksphone:tinderlogin", function(username, passaword)
	...
	local sourcePlayer = tonumber(_source)
	 ...
	 exports.oxmysql:execute("SELECT * FROM gksphone_tinderacc WHERE username = @username AND passaword = @passaword", {['@username'] = username, ['@passaword'] = passaword},
	function(result)
...
AddEventHandler("gksphone:tindernewacc", function(name, username, passaword, date, image, gender)
	...
	if Usercik == nil then
		...
		exports.oxmysql:insert("INSERT INTO gksphone_tinderacc (name, username, passaword, date, image, gender, identifier) VALUES (@name, @username, @passaword, @date, @image, @gender, @identifier)", {
			["@name"] = name,
			["@username"] = username,
			["@passaword"] = passaword,
			["@date"] = date,
...
AddEventHandler("gksphone:tindermatch", function(user_id, friend_id, is_match)
	...
	local matc = exports.oxmysql:executeSync("SELECT is_match FROM gksphone_tindermatch WHERE user_id = @user_id AND friend_id = @friend_id", {['@user_id'] = user_id, ['@friend_id'] = friend_id})

	if matc[1] == nil then
		exports.oxmysql:insert("INSERT INTO gksphone_tindermatch (user_id, friend_id, is_match) VALUES (@user_id, @friend_id, @is_match)", {
			["@user_id"] = user_id,
			["@friend_id"] = friend_id,
			["@is_match"] = is_match
		})
				...
				local result = exports.oxmysql:executeSync("SELECT is_match FROM gksphone_tindermatch WHERE user_id = @user_id AND friend_id = @friend_id AND is_match = 1", {['@user_id'] = user_id, ['@friend_id'] = friend_id})

				Citizen.Wait(1000)

				local result2 = exports.oxmysql:executeSync("SELECT is_match FROM gksphone_tindermatch WHERE user_id = @user_id AND friend_id = @friend_id AND is_match = 1", {['@user_id'] = friend_id, ['@friend_id'] = user_id})
	...
	elseif matc[1].is_match == 0 then
		exports.oxmysql:execute("UPDATE gksphone_tindermatch SET is_match = @is_match WHERE user_id = @user_id AND friend_id = @friend_id", {
			["@user_id"] = user_id,
			["@friend_id"] = friend_id,
			["@is_match"] = is_match
		})
...
AddEventHandler("gksphone:tinderimagechange", function(id, image)
	...
	exports.oxmysql:execute("UPDATE gksphone_tinderacc SET image = @image WHERE id = @id", {
			["@id"] = id,
			["@image"] = image
		})
...
AddEventHandler("gksphone:tinderacdel", function(id)
	...
	if id ~= nil then
		...
		exports.oxmysql:execute("DELETE FROM gksphone_tinderacc WHERE id = @id", {
			["@id"] = id
		})

		exports.oxmysql:execute("DELETE FROM gksphone_tindermatch WHERE user_id = @id OR friend_id = @id", {
			["@id"] = id
		})
...
AddEventHandler("gksphone:tindermessageg", function(message, tinderes, owner)
	...
	exports.oxmysql:insert("INSERT INTO gksphone_tindermessage (message, tinderes, owner) VALUES (@message, @tinderes, @owner)", {
		["@message"] = message,
		["@tinderes"] = tinderes,
		["@owner"] = owner
	})
...
AddEventHandler("gksphone:mailallfetch", function(source)
    ...
    local mails = exports.oxmysql:executeSync("SELECT * FROM gksphone_mails WHERE citizenid = @citizenid ORDER BY time DESC", {
...
AddEventHandler("gksphone:MailDelete", function(id)
	...
	local e = QBCore.Functions.GetPlayer(src)
	if e then
		exports.oxmysql:execute('DELETE FROM gksphone_mails WHERE id = @id', {
			['@id'] = id
		}, function ()
...
AddEventHandler('gksphone:NewMail', function(citizenid, mailData)
    local Player = QBCore.Functions.GetPlayerentifier(citizenid)

    if Player then
        local src = Player.source

		exports.oxmysql:insert("INSERT INTO gksphone_mails (citizenid, sender, subject, image, message) VALUES (@citizenid, @sender, @subject, @image, @message)", {
			["@citizenid"] = citizenid,
...
QBCore.Functions.CreateCallback('gksphone:getphonegallery', function(source, cb)
	local e=QBCore.Functions.GetPlayer(source)
	 exports.oxmysql:execute('SELECT * FROM gksphone_gallery WHERE hex = @identifier ORDER BY TIME DESC',{["@identifier"]=e.identifier},function(result)
		local gallery = {}
		for i=1, #result, 1 do
			table.insert(gallery, {id = result[i].id,	image=result[i].image}) 
		end
...
local e = QBCore.Functions.GetPlayer(src)
if e then
	exports.oxmysql:insert('INSERT INTO gksphone_gallery (`hex`, `image`) VALUES(@identifier, @image);', {
...
local e = QBCore.Functions.GetPlayer(src)
if e then
	exports.oxmysql:insert('DELETE FROM gksphone_gallery WHERE id = @id AND hex = @identifier', {
		['@identifier'] = e.identifier,
		['@id'] = a
	}, function (result)
