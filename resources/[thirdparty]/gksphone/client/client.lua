...
function TooglePhone() 
  QBCore.Functions.TriggerCallback('gksphone:phone-check', function(durum)
    if durum ~= nil then
      ...
      QBCore.Functions.Notify(_U('no_item'), 'error')
    end
  end)
end
...
AddEventHandler("gksphone:waitingCall", function(infoCall, initiator, who)
    --Config.CoreName.Functions.TriggerCallback('gksphone:phone-check', function(durum)

      if flightmode == false then
...
function startCall (phone_number, who, rtcOffer, extraData)
  if rtcOffer == nil then
    rtcOffer = ''
  end

  TriggerServerEvent('gksphone:startCall', phone_number, who, rtcOffer, extraData)
end
...
RegisterNUICallback('startCall', function (data, cb)
  startCall(data.numero, data.who, data.rtcOffer, data.extraData)
  cb()
end)
...
RegisterNUICallback('sendMessages', function(data, cb)
  ...
  TriggerServerEvent('gksphone:sendMessage', data.phoneNumber, data.message)
end)
...
RegisterNUICallback('getPhoneAvatar', function(data, cb)
  QBCore.Functions.TriggerCallback('gksphone:getAvatar', function(avatarr)
...
AddEventHandler('QBCore:Client:OnJobUpdate', function(JobInfo)
  ...
  if JobInfo.isboss then
   ...
   SendNUIMessage({event = 'jobname', jobname = JobInfo.name})
